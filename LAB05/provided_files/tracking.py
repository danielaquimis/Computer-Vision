...
class BoundingEllipse(BoundingBox):
    def __init__(self, center, axes_lengths):
        ...
        self.axes_lengths = np.asarray(axes_lengths)

        x, y = center
        hx, hy = axes_lengths
        super().__init__([x - hx, y - hy, x + hx, y + hy])

    @property
    def hx(self):
        return self.axes_lengths[0]

    @property
    def hy(self):
        return self.axes_lengths[1]

    def __str__(self):
        return f'EllipseBox({self.bbox})'
...
def draw_ellipses(frame, bounding_ellipses, color=None, thickness=2, show_center=True, radius=4, copy=True):
    if copy:
        frame = np.copy(frame)

    if not isinstance(bounding_ellipses, list):
        bounding_ellipses = [bounding_ellipses]
    ...
    for be in bounding_ellipses:
        center = be.center.astype(np.int32)
        axes_lengths = be.axes_lengths.astype(np.int32)
...
def draw_detections(frame, detection_boxes, frame_id=None, colors=None, thickness=2, show_id=True, font_scale=1,
    ...
    for bb in detection_boxes:
        ...
        if isinstance(bb, BoundingEllipse):
            center = bb.center.astype(np.int32)
            axes_lengths = bb.axes_lengths.astype(np.int32)
            angle, start_angle, end_angle = 0, 0, 360
            cv2.ellipse(frame, center, axes_lengths, angle, start_angle, end_angle, color, thickness)
        else:
            cv2.rectangle(frame, (x1, y1), (x2, y2), color, thickness)
